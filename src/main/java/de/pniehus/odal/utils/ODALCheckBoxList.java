/*
 * based on lanterna(https://github.com/mabe02/lanterna) by Martin Berglund
 */
...
public class ODALCheckBoxList<V> extends AbstractListBox<V, ODALCheckBoxList<V>> {
     ...
     */
    public ODALCheckBoxList(TerminalSize preferredSize) {
        super(preferredSize);
    ...
    @Override
    protected ListItemRenderer<V,ODALCheckBoxList<V>> createDefaultListItemRenderer() {
        return new CheckBoxListItemRenderer<V>();
    }

    @Override
    public synchronized ODALCheckBoxList<V> clearItems() {
        itemStatus.clear();
        return super.clearItems();
    }

    @Override
    public ODALCheckBoxList<V> addItem(V object) {
     ...
     * @return Itself
     */
    public synchronized ODALCheckBoxList<V> addItem(V object, boolean checkedState) {
     ...
     * @return Itself
     */
    public synchronized ODALCheckBoxList<V> setChecked(V object, boolean checked) {
    ...
    /**
     * Programmatically sets the checked state of an item in the list box, does not call the listeners onStatusChanged method
     * @param object Object to set the checked state of
     * @param checked If {@code true}, then the item is set to checked, otherwise not
     * @return Itself
     */
    public synchronized ODALCheckBoxList<V> setSilentlyChecked(V object, boolean checked) {
        int index = indexOf(object);
        if(index != -1) {
     ...
     * @return Itself
     */
    public synchronized ODALCheckBoxList<V> addListener(Listener listener) {
     ...
     * @return Itself
     */
    public ODALCheckBoxList<V> removeListener(Listener listener) {
     ...
     * @param <V> Type of items in the {@link ODALCheckBoxList}
     */
    public static class CheckBoxListItemRenderer<V> extends ListItemRenderer<V,ODALCheckBoxList<V>> {
        @Override
        ...
        @Override
        public String getLabel(ODALCheckBoxList<V> listBox, int index, V item) {
            String check = " ";
        ...
        @Override
        public void drawItem(TextGUIGraphics graphics, ODALCheckBoxList<V> listBox, int index, V item, boolean selected, boolean focused) {
            ThemeDefinition themeDefinition = listBox.getTheme().getDefinition(ODALCheckBoxList.class);
